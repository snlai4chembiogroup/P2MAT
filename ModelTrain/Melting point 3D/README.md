...
Melting point 3D/
├── Data/
│   ├── train_set.csv          original training set (3 589 molecules)
│   ├── test_set.csv           original test set     (  897 molecules)
│   ├── train_cho.csv          CHO subset (no N)     – generated  (1 231 train /  297 test)
│   ├── test_cho.csv
│   ├── train_chon.csv         CHON subset (has N)   – generated  (1 562 train /  393 test)
│   ├── test_chon.csv
...
Each dataset uses ~1 625 molecular descriptors (3D conformer-based, computed via Mordred/RDKit 3D) as features
...
> (e.g. `train_set_1.csv` … `train_set_4.csv`).  
