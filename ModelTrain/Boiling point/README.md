# QSAR Boiling Point Prediction Pipeline

A modular machine-learning pipeline for predicting molecular boiling points (BP\_K, Kelvin)
...
Boiling point/
├── Data/
│   ├── train_set.csv          original training set (3 764 molecules)
│   ├── test_set.csv           original test set     (  941 molecules)
│   ├── train_cho.csv          CHO subset (no N)     – generated  (2 303 train)
│   ├── test_cho.csv                                               (  542 test)
│   ├── train_chon.csv         CHON subset (has N)   – generated  (  576 train)
│   ├── test_chon.csv                                              (  161 test)
...
│   └── stacking.py            stacking ensemble over LightGBM + XGBoost + ExtraTrees + DNN
...
|-------|-------------------------------|--------|-------|
| full  | all molecules                 |  3 764 |   941 |
| cho   | nN == 0 (C, H, O only)        |  2 303 |   542 |
| chon  | nN > 0  (C, H, O + N)         |    576 |   161 |

Each dataset uses ~1 217 molecular descriptors (PaDEL / CDK) as features
and `BP_K` (boiling point in Kelvin) as the target variable.

Dropped columns: `#PubChem`, `BP_C`, `smiles`  
(identifier, Celsius duplicate of the target, and SMILES string)

> **Note:** Large CSV files in `Data/` are split into numbered parts for GitHub
> (e.g. `train_set_1.csv`, `train_set_2.csv`).  
...
### 2. Merge split data files

Large CSV files are stored as numbered parts on GitHub. Reconstruct them before training:
...
### 3. Generate CHO / CHON subsets

```bash
conda run -n qsar python split_datasets.py
```

### 4. Run the full pipeline (216 experiments)

```bash
conda run -n qsar python main.py
```

### 5. Run a subset (e.g. quick test)
...
### 6. Run datasets sequentially across separate sessions
...
### 7. Automatic load-or-train logic (default behaviour)
...
### 8. Run the stacking ensemble

Stacking requires the base models (LightGBM, XGBoost, ExtraTrees, DNN) to already be trained
...
1. **Load base-model artifacts** — for each (dataset, transform, fs_method) combo,
   load the four saved `pipeline.joblib` files produced by `main.py`.
...
6. **Outputs**:
   ...
   - `results/outliers_smiles_{dataset}.csv` — flagged training compounds
     with SMILES, true/predicted BP, residual, standardised residual,
